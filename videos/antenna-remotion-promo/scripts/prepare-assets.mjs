...
const capturesDir = path.join(projectDir, "public", "captures");
...
const captures = [
  {
    role: "dashboard-interaction",
    source: "videos/antenna-hyperframes-promo/assets/dashboard-interaction.mp4",
    dest: "dashboard-interaction.mp4",
  },
];
...
for (const capture of captures) {
  const sourcePath = path.join(repoRoot, capture.source);
  const destPath = path.join(capturesDir, capture.dest);
  const sourceInfo = await stat(sourcePath);
  if (!sourceInfo.isFile() || sourceInfo.size === 0) {
    throw new Error(`Source capture is missing or empty: ${sourcePath}`);
  }
  await copyFile(sourcePath, destPath);
  const destInfo = await stat(destPath);
  captureManifest.push({
    ...
    file: `captures/${capture.dest}`,
    bytes: destInfo.size,
  });
}

await writeFile(
  path.join(capturesDir, "capture-manifest.json"),
  `${JSON.stringify({ captures: captureManifest }, null, 2)}\n`,
);

console.log(`Copied ${manifest.length} assets into ${assetsDir}`);
console.log(`Copied ${captureManifest.length} captures into ${capturesDir}`);
