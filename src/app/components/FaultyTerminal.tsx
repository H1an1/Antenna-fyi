...
float digit(vec2 p){
  ...
  float n = i * i + j * j;
  float f = n * 0.0625;
  // Modulate intensity with hand luminance — bright skin = sparse, dark robe = dense
  if (uUseMask > 0.5) {
    float lum = texture2D(uLum, vUv).r;
    // Invert: bright areas reduce intensity (sparser), dark areas increase it (denser)
    intensity = intensity * (0.3 + 1.4 * (1.0 - lum));
  }
...
export default function FaultyTerminal({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({ dpr: actualDpr, alpha: true, premultipliedAlpha: false });
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uMask: { value: new Texture(gl) },
        uLum: { value: new Texture(gl) },
        uUseMask: { value: maskUrl ? 1 : 0 },
      },
    });
    ...
    if (maskUrl) {
      ...
      img.onload = () => {
        program.uniforms.uMask.value = new Texture(gl, { image: img });
      };
      img.src = maskUrl;
    }
    // Load luminance texture
    if (lumUrl) {
      const img2 = new Image();
      img2.crossOrigin = "anonymous";
      img2.onload = () => {
        program.uniforms.uLum.value = new Texture(gl, { image: img2 });
      };
      img2.src = lumUrl;
    }
  ...
  }, [actualDpr, pause, timeScale, scale, gridMul, digitSize, scanlineIntensity, glitchAmount, flickerAmount, noiseAmp, chromaticAberration, ditherValue, curvature, tintVec, mouseReact, mouseStrength, pageLoadAnimation, brightness, maskUrl, lumUrl, handleMouseMove]);
